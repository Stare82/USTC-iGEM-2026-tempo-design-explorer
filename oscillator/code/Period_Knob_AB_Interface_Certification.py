"""Certify the calibrated period-knob library against Zhao module B.

The script reads the representative period-knob library produced by
``Shared_PLtetO1_Multiobjective_Period_Knob_Designer.py``.  For every period
setting it re-solves module A, converts the actual C31 translation flux
...
Run from the project root:
    ...
    python ./code/Period_Knob_AB_Interface_Certification.py --mode smoke
    python ./code/Period_Knob_AB_Interface_Certification.py --mode full
...
VERSION = "Period_Knob_AB_Interface_Certification_20260819"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Period-knob representative library; newest matching table is used by default.",
    )
    parser.add_argument(
        "--b-model-dir",
        type=Path,
...
def locate_representative_csv(explicit: Path | None) -> Path:
    if explicit is not None:
        ...
        return path
    matches = sorted(
        OUTPUT_ROOT.glob("Multiobjective_Period_Knob_*/period_knob_representative_library.csv"),
        key=lambda path: path.stat().st_mtime,
        reverse=True,
    )
    if not matches:
        raise FileNotFoundError(
            "No period-knob representative library was found. Pass --representative-csv."
        )
    return matches[0]
...
def main() -> None:
    ...
    representatives = pd.read_csv(representative_csv)
    required = {
        "target_period_generations",
        "period_generations",
    ...
    output_dir = OUTPUT_ROOT / f"Period_Knob_AB_Certification_{args.mode}_{stamp}"
    figure_dir = FIGURE_ROOT / f"Period_Knob_AB_Certification_{args.mode}_{stamp}"
    ...
    metadata = {
        ...
        "interpretation": [
            ...
            "The 8-generation source point remains uncertified if its period error exceeds tolerance.",
